...
export default function Modal({
  ...
  const validate = () => {
    ...
    for (const field of tempErrors) {
      if (formData[field.id] === undefined) {
        ...
        field.type === "select" &&
        formData[field.id] === "Pilih status..."
      ) {
        isValid = false;
        newErrors[field.id] = `${field.label} harus diisi.`;
      }
  ...
  return (
    <dialog id="" className="modal modal-open">
      <div className="modal-box h-4/5 md:w-300 p-9 px-4 bg-white">
        <h3 className="font-bold text-[#6D123F] text-lg text-center ">
          Formulir Pendaftaran Bantuan
        </h3>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-action mt-6 px-4">
            {currentStep > 1 && (
              <button
                type="button"
                className="btn bg-gray-300 rounded-lg text-sm"
                onClick={handleBack}
              >
                Kembali
              </button>
            ...
            {currentStep < totalSteps && (
              <button
                type="button"
                className="btn bg-[#6D123F] rounded-xl text-white border-none hover:bg-pink-600"
                onClick={handleNext}
              >
                Lanjut
              </button>
